-- ============================================================
--  Anvil Trades Portal — Postgres schema
--
--  Keep this in step with schema.sqlite.sql. The differences
...
DROP VIEW  IF EXISTS open_jobs CASCADE;
...
CREATE TABLE quiz_results (
  ...
  answers     TEXT NOT NULL,  -- JSON array; SQLite adds a json_valid CHECK here
...
CREATE TABLE applications (
  ...
  applied_date    TEXT NOT NULL DEFAULT to_char(now(), 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE TABLE resumes (
  ...
  model        TEXT,
  analyzed_at  TEXT,
  analysis     TEXT,          -- JSON object; SQLite adds a json_valid CHECK here
  FOREIGN KEY (student_id) REFERENCES students (student_id) ON DELETE CASCADE
);
...
-- ------------------------------------------------------------
CREATE UNIQUE INDEX idx_users_email ON users (lower(email));  -- SQLite gets this from COLLATE NOCASE
